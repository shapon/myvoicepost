...
function UsersTab() {
  ...
  const { data, isLoading, refetch } = useQuery<any>({
    queryKey: ["/api/v1/a/admin/users", `?page=${page}&limit=15`],
  });
...
function SubscriptionsTab() {
  ...
  const { data, isLoading, refetch } = useQuery<any>({
    queryKey: ["/api/v1/a/admin/subscriptions", `?page=${page}&limit=15`],
  });
...
function PaymentsTab() {
  ...
  const { data, isLoading, refetch } = useQuery<any>({
    queryKey: ["/api/v1/a/admin/payments", query],
  });
...
function SupportTab() {
  ...
  const { data, isLoading, refetch } = useQuery<any>({
    queryKey: ["/api/v1/a/admin/support", `?page=${page}&limit=15${filterQuery}`],
  });

  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const res = await apiRequest("PATCH", `/api/v1/a/admin/support/${id}`, { status });
      if (!res.ok) throw new Error("Failed to update");
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/v1/a/admin/support"] });
      queryClient.invalidateQueries({ queryKey: ["/api/v1/a/admin/stats"] });
    },
  });
...
function ErrorsTab() {
  ...
  const { data, isLoading, refetch } = useQuery<any>({
    queryKey: ["/api/v1/a/admin/errors", `?page=${page}&limit=15`],
  });
